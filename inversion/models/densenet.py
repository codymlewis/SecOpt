...
class ConvBlock(nn.Module):
    ...
    def __call__(self, x, train=True):
        x1 = nn.LayerNorm(epsilon=1.001e-5, name=self.name + '_0_gn')(x)
        x1 = nn.relu(x1)
        x1 = nn.Conv(4 * self.growth_rate, (1, 1), padding='VALID', use_bias=False, name=self.name + '_1_conv')(x1)
        x1 = nn.LayerNorm(epsilon=1.001e-5, name=self.name + '_1_gn')(x1)
        x1 = nn.relu(x1)
...
class TransitionBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, train=True):
        x = nn.LayerNorm(epsilon=1.001e-5, name=self.name + '_gn')(x)
        x = nn.relu(x)
...
class DenseNet(nn.Module):
    ...
    def __call__(self, x, representation=False, train=True):
        ...
        x = nn.LayerNorm(epsilon=1.001e-5, name='conv1/gn')(x)
        x = nn.relu(x)
        ...
        x = nn.LayerNorm(epsilon=1.001e-5, name="gn")(x)
        x = nn.relu(x)
...
class DenseNet121(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, representation=False, train=True):
        return DenseNet(self.classes, [6, 12, 24, 16])(x, representation, train)
