...
# Note: The group norms require specialized training functions, checkout the resnetrs branch
...
class BottleneckBlock(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = nn.LayerNorm(
            ...
            name=f"{self.name}_group_norm_2"
        )(x)
        x = nn.relu(x)
        x = Conv2DFixedPadding(
        ...
        x = nn.LayerNorm(
            momentum=self.bn_momentum,
            epsilon=self.bn_epsilon,
            name=f"{self.name}_group_norm_3"
        )(x)
...
class ResNetRS(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, representation=False, train=True):
        x = STEM(
