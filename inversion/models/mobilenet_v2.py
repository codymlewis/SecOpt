...
class InvertedResBlock(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = nn.LayerNorm(
            epsilon=1e-3, momentum=0.999, name=prefix + "depthwise_GN"
        )(x)
        x = nn.relu6(x)
        x = nn.Conv(
...
class MobileNetV2(nn.Module):
    ...
    def __call__(self, x, representation=False, train=True):
        ...
        x = nn.LayerNorm(epsilon=1e-3, momentum=0.999, name="gn_Conv1")(x)
        x = nn.relu6(x)
        ...
        x = nn.Group(
            axis=-1, epsilon=1e-3, momentum=0.999, name="Conv_1_gn"
        )(x)
        x = nn.relu6(x)
