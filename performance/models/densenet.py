...
class ConvBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        x1 = nn.LayerNorm(epsilon=1.001e-5, name=self.name + '_0_ln')(x)
        x1 = nn.relu(x1)
        x1 = nn.Conv(4 * self.growth_rate, (1, 1), padding='VALID', use_bias=False, name=self.name + '_1_conv')(x1)
        x1 = nn.LayerNorm(epsilon=1.001e-5, name=self.name + '_1_ln')(x1)
        x1 = nn.relu(x1)
...
class TransitionBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        x = nn.LayerNorm(epsilon=1.001e-5, name=self.name + '_ln')(x)
        x = nn.relu(x)
...
class DenseNet(nn.Module):
    ...
    def __call__(self, x, representation=False):
        ...
        x = nn.LayerNorm(epsilon=1.001e-5, name='conv1/ln')(x)
        x = nn.relu(x)
        ...
        x = nn.LayerNorm(epsilon=1.001e-5, name="ln")(x)
        x = nn.relu(x)
...
class DenseNet121(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, representation=False):
        return DenseNet(self.classes, [6, 12, 24, 16])(x, representation)
...
class DenseNet169(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, representation=False):
        return DenseNet(self.classes, [6, 12, 32, 32])(x, representation)
...
class DenseNet201(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, representation=False):
        return DenseNet(self.classes, [6, 12, 48, 32])(x, representation)
