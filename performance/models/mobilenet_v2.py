...
class InvertedResBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.LayerNorm(
            epsilon=1e-3, momentum=0.999, name=prefix + "depthwise_LN"
        )(x)
        x = nn.relu6(x)
        x = nn.Conv(
...
class MobileNetV2(nn.Module):
    ...
    def __call__(self, x, representation=False):
        ...
        x = nn.LayerNorm(epsilon=1e-3, momentum=0.999, name="ln_Conv1")(x)
        x = nn.relu6(x)
        ...
        x = nn.LayerNorm(
            epsilon=1e-3, momentum=0.999, name="Conv_1_ln"
        )(x)
        x = nn.relu6(x)
